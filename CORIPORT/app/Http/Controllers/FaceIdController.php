...
class FaceIdController extends Controller{
    ...
    public function store(Request $request)
    {
        try {
            ...
            if (!empty($data)) {
                ...
                if (!$valid->fails()) {
                    $faceId = new FaceId();
                    $faceId->imageData = $data['imageData'];
            ...
            if ($valid->fails()) {
                ...
                if (!empty($id)) {
                    ...
                    if ($faceId) {
                        ...
                        $response = array(
                            'status' => 400,
                            'message' => 'El FaceId del empleado no existe',
                        );
                    }
                } else {
                    $response = array(
                        'status' => 400,
                        'message' => 'El ID del FaceId no es válido',
                    );
                }
            }
        }
    ...
    {
        if (isset($id)) {
            $deleted = FaceId::where('id', $id)->delete();
            if ($deleted) {
                $response = array(
                    'status' => 200,
                    'message' => 'El FaceId del empleado se eliminado correctamente'
                );
            } else {
                $response = array(
                    'status' => 400,
    ...
    public function show($id)
    {
        ...
        if (is_object($faceId)) {
            ...
            $response = [
                'status' => 404,
                'message' => 'El FaceId no se encuentra',
            ];
        }
        return response()->json($response, $response['status']);
    }
